...
class CircularGraphGUI:
    ...
    def __init__(self):
        ...
        self.root.geometry('700x500') # Window size
    ...
    def create_widgets(self):
        ...
        # Create Gui Title
        title = tk.Label(self.root, text='Circular Graph Plotter', font=('Calibri Light', 20, 'bold'))
        ...
        self.mat_entry.grid(row=2, column=0, padx=20, pady=2)
        self.mat_browse_button = tk.Button(self.root, text='Browse...', command=self.browse_mat_file)
        self.mat_browse_button.grid(row=2, column=0, sticky='w', padx=(560, 0), pady=2)
        
        self.file_type = tk.StringVar(value='')
        self.connmat_rb = tk.Radiobutton(self.root, text='Connectivity matrix', variable=self.file_type, value='conn_mat')
        ...
        self.other_1_entry.bind('<FocusIn>', self.clear_placeholder)
        self.other_1_entry.bind('<FocusOut>', self.restore_placeholder)

        self.atlas_1_choice.bind('<<ComboboxSelected>>', self.show_other_1_entry)
        ...
        self.other_2_entry.bind('<FocusIn>', self.clear_placeholder)
        self.other_2_entry.bind('<FocusOut>', self.restore_placeholder)

        self.atlas_2_choice.bind('<<ComboboxSelected>>', self.show_other_2_entry)
        ...
        self.color_scheme_button = tk.Button(self.root, text='Select Color Scheme', command=self.select_color_scheme)
    ...
    def show_other_1_entry(self, event):
    ...
    def show_other_2_entry(self, event):
    ...
    def clear_placeholder(self, event):
        if self.other_entry.get() == 'Enter path here..':
            self.other_entry.delete(0, tk.END)
            self.other_entry.config(fg='black')

    def restore_placeholder(self, event):
        if self.other_entry.get() == "":
            self.other_entry.insert(0, 'Enter path here..')
            self.other_entry.config(fg='gray')
    ...
    def update_secondary_options(self, clicked):
        if clicked == 'none' and self.none_var.get():
            ...
            self.color_scheme_button.grid_remove()

        elif clicked in ('color', 'bracket'):
            if self.color_var.get():
                self.none_var.set(False)
                self.color_scheme_button.grid(row=8, column=0, sticky='w', padx=(450, 0), pady=2)
    ...
    def select_color_scheme(self):
        ...
        self.color_window.title('Color Scheme')
        self.color_window.geometry('480x100')

        tk.Label(self.color_window, text='Path to color scheme CSV:').grid(row=0, column=0, sticky='w', padx=(10, 0), pady=2)

        self.color_scheme_entry = tk.Entry(self.color_window, width=40)
        self.color_scheme_entry.grid(row=0, column=0, sticky='w', padx=(160, 0), pady=2)

        tk.Button(self.color_window, text='Browse...', command=self.browse_color_scheme).grid(row=0, column=0, sticky='w', padx=(410, 0), pady=2)
        ...
        tk.Button(self.color_window, text='Done', command=self.save_color_scheme).grid(row=2, column=0, sticky='w', padx=(227, 0), pady=2)

    def browse_color_scheme(self):
        filename = filedialog.askopenfilename(filetypes=[('CSV files', '*.csv')])

        if filename:
            self.color_scheme_entry.delete(0, tk.END)
            self.color_scheme_entry.insert(0, filename)
    ...
    def plot_circular_graph(self):
        self.attributes = {
            ...
            'first_level_labeling': self.labeling_choice.get() == 'True',
            'color_palette': self.color_palette_path,
            'secondary_color': self.color_var.get(),
            'secondary_grouping': self.grouping_var.get(),
        ...
        self.attributes['labels'] = self.get_atlas(self.atlas_1_choice.get(), self.other_1_entry.get().strip())
        self.attributes['secondary_labels'] = self.get_atlas(self.atlas_2_choice.get(), self.other_2_entry.get().strip())

        # cg_object = CircularGraph(
        #             mat_path=self.attributes['mat_path'],
        #             mat_type=self.attributes['mat_type'],
        #             labels=self.attributes['labels'],
        #             secondary_labels=self.attributes['secondary_labels'],
